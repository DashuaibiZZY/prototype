...
<<<<<<< HEAD
        - **系统通知**：见《[事件目录PRD](事件目录PRD.md)》§3.1 邀请返佣（4 条事件）。缓冲期第 5、7 天倒计时 **不发** 系统通知。
=======
...
<<<<<<< HEAD
    - **数据来源**：后台「邀请返佣管理」提交回收指令，并在 **次日 0:00（UTC+8）** 批次 **执行成功** 后写入用户可见记录，并同步触发 `invite_rebate.reclaimed`（见《[事件目录PRD](事件目录PRD.md)》§3.1）
=======
    - **数据来源**：后台「邀请返佣管理」提交回收指令，并在 **次日 0:00（UTC+8）** 批次 **执行成功** 后写入用户可见记录，并同步触发消息事件 `invite_rebate.reclaimed`（见 [消息中心 · 邀请返佣事件](https://ksgf73ukjokp.sg.larksuite.com/wiki/BiTown38EipLF3k8TJllGd35gHc)）
>>>>>>> origin/main
...
> 子页面结构、申请表单字段、五态规则及后台审核流程见《[邀请合伙人无限层返佣系统](邀请合伙人无限层返佣系统.md)》§2.8、§2.7。
...
> 子页面详情见《[邀请合伙人无限层返佣系统](邀请合伙人无限层返佣系统.md)》§2.8（含五态与审核结果通知规则）。
...
| 回收原因 | 必填 · 多行 | 写入消息事件 `invite_rebate.reclaimed` 的 `reclaim_reason` 变量 |

**交互规则**：

- 运营点击【确认】后，系统创建 **待执行回收指令**，挂入 **次日 0:00（UTC+8）** 日结批次队列。
- **当日 0:00 前**（指令所属批次执行前）：允许修改回收金额或撤销指令。
- **过 0 点批次执行后**：回收金额 **不可变更**；系统扣减用户待结算返佣、触发消息事件 `invite_rebate.reclaimed`，并在用户端 **待结算回收记录** 写入一条对外记录（Web 弹窗 / App 子页面，见本文 §2\.2\.1、§5\.1）。
...
<<<<<<< HEAD
事件变量、消息模板与幂等规则见《[事件目录PRD](事件目录PRD.md)》§3.1 · `invite_rebate.reclaimed`。
=======
