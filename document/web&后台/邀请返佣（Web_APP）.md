...
> **交互原型**：纵向全宽画框平铺（见上方链接），含普通用户、分享海报弹窗、合伙人等状态。

> **导航入口**：全局顶部导航 “邀請返佣”（选中态加粗黑色 \+ 底部 2px 黑色下划线）

> **页面状态**：需连接钱包后方可进入邀请返佣页；未连接钱包访问本页路由时跳转并引导连接

> **邀请链接落地（注意事项）**：用户打开推广链接（URL 含 `ref` 邀请码，如 `forx.finance/?ref=E6DL28G`）时，**落地页为 Web 合约交易页**，并自动弹出邀请注册引导弹窗——展示邀请码、注册引导文案与「连接钱包」按钮；用户点击后进入钱包连接与注册/登录流程，邀请关系在注册环节自动绑定。原型见 `app/邀请返佣.html` 平铺页（手机框示意 Web 交互，非 APP 内页面）。
...
- **交互原型**：手机框平铺（见上方链接），含普通用户、合伙人、分享海报弹窗、说明浮窗、邀请链接落地示意等状态。
...
- **原型位置**：`app/邀请返佣.html` 平铺原型 · 邀请链接落地弹窗示意
