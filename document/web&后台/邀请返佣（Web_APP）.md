...
> **原型状态索引**（`邀请返佣.html`）：① 普通用户 · 默认（§2 全文） · ② 分享海报弹窗（§2\.6） · ③ 合伙人用户

> **导航入口**：全局顶部导航 “邀請返佣”（选中态加粗黑色 \+ 底部 2px 黑色下划线）

> **页面状态**：需连接钱包后方可进入邀请返佣页；未连接钱包访问本页路由时跳转并引导连接

> **邀请链接落地（注意事项）**：用户打开推广链接（URL 含 `ref` 邀请码，如 `forx.finance/?ref=E6DL28G`）时，**落地页为 Web 合约交易页**，并自动弹出邀请注册引导弹窗——展示邀请码、注册引导文案与「连接钱包」按钮；用户点击后进入钱包连接与注册/登录流程，邀请关系在注册环节自动绑定。原型示意见 `app/邀请返佣.html` 状态 **⑥ 邀请链接落地 · Web 交易页弹窗**（以手机框示意 Web 交互，非 APP 内页面）。
...
- **原型状态索引**（`app/邀请返佣.html`）：① 普通用户 · ② 合伙人用户 · ③ 分享海报弹窗（§8） · ④ 说明浮窗 · 待结算收益 · ⑤ 说明浮窗 · 活跃好友（§10） · **⑥ 邀请链接落地 · Web 交易页弹窗**（§9）
...
- **场景**：访客通过推广链接（`?ref=邀请码`）打开 **Web 合约交易页** 时自动弹出（非 APP 内页面）

- **原型位置**：`app/邀请返佣.html` 状态 **⑥ 邀请链接落地 · Web 交易页弹窗**
