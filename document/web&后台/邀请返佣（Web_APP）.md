...
> **邀请链接落地（注意事项）**：用户打开推广链接（URL 含 `ref` 邀请码，如 `forx.finance/?ref=E6DL28G`）时，**落地页为 Web 合约交易页**，并自动弹出邀请注册引导弹窗——展示邀请码、注册引导文案与「连接钱包」按钮；用户点击后进入钱包连接与注册/登录流程，邀请关系在注册环节自动绑定。原型示意见 `app/邀请返佣.html`（以手机框示意 Web 交互，非 APP 内页面）。
