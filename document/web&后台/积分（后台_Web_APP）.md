...
| 切换积分计划总开关 | 「每周积分总池设置」Tab **写**；确认后立即生效，不走审批 |
...
位于页面顶部独立卡片，控制 **每周积分总池自动瓜分（机制 3）** 是否执行。与总池数值/占比配置 **独立**：开关变更 **确认后立即生效**，**不走** 总池配置审批流。

| 配置项 | 说明 |
|---|---|
| 开关 | **开启** / **关闭**（Toggle） |
| 当前状态 | 发放中 / 已暂停 |
| 确认弹窗 | 切换前须二次确认，并说明结算周边界影响 |

**结算周边界规则**（周一 00:00 UTC+8 为结算点）：

| 操作 | 本周（含即将执行的一次周结算） | 自下周起 | 前端积分页 |
|---|---|---|---|
| **开启 → 关闭** | 周结算 **正常执行** | **暂停** 每周积分发放 | 立即展示暂停提示 |
| **关闭 → 开启** | **跳过** 本周周结算 | **恢复** 每周积分发放 | 立即恢复正常展示 |
...
**与积分计划总开关的关系**：总池数值/占比变更仍走上述审批；**总开关**独立配置，确认后立即生效，写入操作记录（类型：规则配置），**不生成**审批单。
...
|操作|记录内容|
|---|---|
|切换积分计划总开关|立即写入；详情如 `ForX 积分计划 · 关闭（下周起暂停周结算）` / `…开启（下周起恢复周结算）`|
