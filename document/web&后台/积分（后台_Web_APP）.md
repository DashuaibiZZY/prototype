...
后台采用 **ForX Admin 统一侧栏**（见《[后台权限与角色管理](后台权限与角色管理.md)》§3）。除各业务模块外，顶部 **系统 → 权限与用户** 为全局权限配置入口。
...
**权限**：积分管理员（提交申请）；市场运营交叉 / 风控 / 老板（按角色审批对应节点）。权限码与默认角色矩阵见《[后台权限与角色管理](后台权限与角色管理.md)》§5.7、§6。原型业务页 **全量展示 UI**，真实显隐以配置后台矩阵 + 生产登录态为准。
