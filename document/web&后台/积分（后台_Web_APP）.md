...
| 切换积分计划总开关 | 「每周积分总池设置」Tab **写**；提交后走 **交叉 + 风控** 审批（无老板） |
| 提交总池配置 / 加成 / 手动发放审批 | 对应 Tab **写**；总池与总开关均为 **交叉 + 风控**（无老板），加成/手动为四级含老板 |
...
位于页面顶部独立卡片，控制 **每周积分总池自动瓜分（机制 3）** 是否执行。与总池数值/占比配置 **独立提交**，但审批流程相同。
...
| 申请备注 | **必填** |
| 确认弹窗 | 展示结算边界影响 + 三级审批预览（积分管理员提交 → 交叉 → 风控） |
| 生效时机 | **风控审核通过后**立即写入配置；前端状态同步更新 |

**审批流程**：与「积分总池配置」相同 — **市场运营交叉审核 → 风控审核**，**无老板审批、不同步 Lark**。审批通过前开关保持原状态，Toggle 禁用。

**结算周边界规则**（周一 00:00 UTC+8 为结算点，**以风控通过时刻所属周为界**）：

| 操作 | 本周（含即将执行的一次周结算） | 自下周起 | 前端积分页 |
|---|---|---|---|
| **开启 → 关闭** | 周结算 **正常执行** | **暂停** 每周积分发放 | 风控通过后立即展示暂停提示 |
| **关闭 → 开启** | **跳过** 本周周结算 | **恢复** 每周积分发放 | 风控通过后立即恢复正常展示 |
...
| 类型 | 审批单类型 | 生效内容 |
|---|---|---|
| 每周总池 / 六维度占比 / 有效持仓最小时长 | 积分总池配置 | 风控通过后写入总池配置，下一结算周按新规则瓜分 |
| ForX 积分计划总开关 | 积分计划总开关 | 风控通过后写入开启/关闭状态，前端同步；结算边界见上表 |
...
积分审核页将**积分手动发放**、**积分加成配置**、**积分总池配置**与**积分计划总开关**四类审批合并为统一列表。手动发放与加成配置沿用四级审批（含老板）；**积分总池配置**与**积分计划总开关**仅需市场运营交叉审核 + 风控审核，**无需老板审批**。

#### 审批视角切换（原型）

页面顶部 Tab 仅用于原型演示。正式上线：组内随机派单（见《[后台权限与人员管理](后台权限与角色管理.md)》§2.4）。**积分总池配置**与**积分计划总开关**无 BOSS 节点。
...
- **积分总池配置 / 积分计划总开关**：三级审批步骤条（积分管理员提交 → 市场运营交叉审核 → 风控审核），**无老板节点、不同步 Lark**。
...
**Lark 联动**：仅适用于需老板审批的类型；风控通过后同步至 Lark，老板可在 Lark 或后台任一端审批。**积分总池配置与积分计划总开关不涉及 Lark。**
...
|操作|记录内容|
|---|---|
|提交积分计划总开关审批|立即写入，状态「审批中」及审批单号；风控通过后更新开关状态与记录|
