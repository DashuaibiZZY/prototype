...
后台采用 **ForX Admin 统一侧栏**，积分模块包含八个子页面：

|侧栏入口|页面|Hash|
|---|---|---|
|积分发放总览|全站发放统计、真实/影子用户拆分|`#overview`|
|每周发放记录|系统周结算与手动发放批次列表 \+ 详情|`#weekly`|
...
**原型**：[https://dashuaibizzy\.github\.io/prototype/perp\_dex/admin/积分后台\.html\#overview](https://dashuaibizzy.github.io/prototype/perp_dex/admin/%E7%A7%AF%E5%88%86%E5%90%8E%E5%8F%B0.html#overview)

面向运营与风控的**全站积分发放仪表盘**，汇总历史与近期发放情况，并区分真实用户与排行榜影子用户。

#### 时间范围

顶部支持切换统计周期：**全部** / **近 30 天** / **近 90 天** / **自定义日期**（默认「全部」）。下列指标均随所选周期刷新。

#### 核心指标卡片

|指标|说明|
|---|---|
|累计发放总积分|所选周期内所有来源写入用户侧的发放合计（含系统周结算、手动发放、新人任务等）|
|真实用户获得积分|计入真实用户账户的积分合计|
|影子用户分摊积分|周结算中影子用户按公式分摊的积分合计（**不进入真实用户余额**，为稀释量）|
|获得积分去重用户数（真实）|所选周期内至少获得 1 次积分的真实用户去重人数|
|影子用户参与人数|所选周期内参与周结算瓜分的影子用户去重人数|
|全站用户积分余额合计|当前时刻所有真实用户积分余额之和（存量，非周期筛选）|

#### 补充指标（建议一并展示）

|指标|说明|
|---|---|
|系统周结算发放合计|周期内周一自动结算发放积分合计|
|手动发放合计|周期内经审批通过的手动发放合计|
|新人任务发放合计|周期内新人任务 A/B 自动发放合计|
|本周计划总池 vs 实发|最近一个结算周：配置总池、实发积分、截断尾差（计划 − 实发）|
|影子分摊占比|周期内影子分摊积分 ÷ 周结算计划总池（反映稀释力度）|
|人均获得积分（真实用户）|真实用户获得积分 ÷ 获得积分去重真实用户数|

#### 发放构成

饼图或条形图展示：系统周结算 / 手动发放 / 新人任务占比；周结算内可再拆真实用户 vs 影子分摊。

#### 快捷入口

- 「查看每周发放记录」跳转 §3\.8 列表，并带上当前时间筛选。

- 「导出总览摘要」：导出当前周期核心指标 CSV（一行多列），便于周报归档。

---

### 3\.8 每周发放记录

**原型**：[https://dashuaibizzy\.github\.io/prototype/perp\_dex/admin/积分后台\.html\#weekly](https://dashuaibizzy.github.io/prototype/perp_dex/admin/%E7%A7%AF%E5%88%86%E5%90%8E%E5%8F%B0.html#weekly)

按**发放批次**汇总系统每周自动结算与手动发放，支持筛选、查看详情与导出。
...
|发放时间|日期范围|
|UID / 钱包|在批次详情内二次筛选（列表页可选：仅筛含该用户的批次）|

列表右侧提供 **下载 CSV**：导出当前筛选条件下的批次汇总列表（非用户明细）。

#### 列表字段

|列|说明|
|---|---|
|发放时间|批次完成时间，如 `2026-07-21 00:05`|
|发放类型|系统周结算 / 手动发放|
|结算周 / 活动|周结算：`2026-W29 (07/14 - 07/20)`；手动：活动名称|
|计划发放|周结算为当周配置总池；手动为批次申请总积分|
|实发积分|实际写入账户（真实用户）的积分合计；周结算另展示影子分摊列|
|影子分摊积分|仅周结算：影子用户分摊合计（不入账）|
|截断尾差|计划发放 − 实发 − 影子分摊（周结算截断剩余）|
|发放用户数|本批次涉及用户总数（真实 \+ 影子）|
|真实用户数 / 影子用户数|周结算批次拆分展示；手动发放仅真实用户|
|操作|「查看详情」|

#### 发放详情页

Hash：`#weekly-detail=<批次ID>`，如 `#weekly-detail=WK2026-W29`。
...
|结算周或活动|周结算周期或手动活动信息|
|计划发放 / 实发 / 影子分摊 / 尾差|与列表一致|
|真实 / 影子用户数|本批次人数拆分|
|关联审批单|手动发放展示 APR 单号；周结算可为 `—`|
|当周子池配置快照|仅周结算：六维度占比 \+ 有效持仓最小时长（结算生效配置）|

##### 发放明细列表

支持 **UID / 钱包** 模糊查询；查询栏旁 **下载 CSV** 导出当前筛选后的用户明细（含六维字段）。

|列|说明|
|---|---|
|UID|用户 UID；影子用户可带「影子」标签|
|钱包地址|EVM 地址|
|用户类型|真实用户 / 影子用户|
|积分加成|当周结算使用的个人等级加成系数，如 `1.2x`|
|本周所得积分|本批次该用户获得的积分合计|
|交易积分|原始数据值 · 子池占比 · 所得积分（三列或合并展示）|
|有效持仓积分|原始数据 · 占比 · 所得积分|
|持仓亏损激励|原始数据 · 占比 · 所得积分|
|持仓盈利额外|原始数据 · 占比 · 所得积分|
|平均资沉积分|原始数据 · 占比 · 所得积分|
...
**原始数据**：该维度用于瓜分的原始数值（如净手续费、有效持仓金额等）。**子池占比**：当周该维度占总池百分比。**所得积分**：该维度截断后的积分。

##### 补充说明（建议实现）

|项|说明|
|---|---|
|分页|明细列表默认每页 50 条，可切换 20 / 50 / 100|
|仅真实用户筛选|快捷筛「隐藏影子用户」，便于核对真实发放|
|手动发放明细|无六维拆分，列表为 UID、钱包、发放积分、活动、审批单号|
|导出字段|CSV 含上述全部列；影子用户行 `用户类型=影子`，便于财务核对|
