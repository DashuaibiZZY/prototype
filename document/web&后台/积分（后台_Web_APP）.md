...
后台采用 **ForX Admin 统一侧栏**（见《[后台权限与角色管理](后台权限与角色管理.md)》§5）。除各业务模块外，顶部 **系统 → 权限与用户** 为全局权限配置入口。
...
**权限**：运营（提交手动发放申请）；市场 / 风控 / 老板（按岗位审批对应节点）。权限清单与内置角色默认矩阵见《[后台权限与角色管理](后台权限与角色管理.md)》§7.6、§8；页面元素与权限对应见 §10.7。原型业务页 **全量展示 UI**，正式上线以登录账号权限为准。
