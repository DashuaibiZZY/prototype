...
后台采用 **ForX Admin 统一侧栏**，积分模块包含七个子页面：
...
单页包含 **上方总览指标** 与 **下方每周发放记录列表**；点击「查看详情」进入发放明细子页（Hash：`#weekly-detail=<批次ID>`）。本页**不提供导出**；用户明细仅在发放详情页支持下载 CSV。

#### 时间范围

顶部支持切换统计周期：**全部** / **近 30 天** / **近 90 天**（默认「全部」）。总览指标随所选周期刷新；下方发放记录列表展示全部历史批次（可用筛选缩小范围）。

#### 核心指标卡片

|指标|说明|
|---|---|
|累计发放总积分|所选周期内所有来源的发放合计（系统周结算、手动发放、新人任务等）|
|真实用户获得积分|计入真实用户账户的积分合计|
|影子用户分摊积分|周结算中影子用户分摊合计（**不入真实用户账户**，为稀释量）|
...
|区块|说明|
|---|---|
|发放来源构成|系统周结算 / 手动发放 / 新人任务占比|
|最近一周结算|计划总池、实发（真实用户）、影子分摊、影子分摊占比|
