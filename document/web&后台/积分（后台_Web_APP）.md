...
本页用于配置每周自动瓜分的积分总池、六维度子池占比及有效持仓最小时长，并展示个人等级加成系数及新人任务规则（系统只读）。系统每周一 **00:00（UTC\+8）** 根据上周数据自动结算发放。
...
各维度子池占比可在本页编辑；**保存子池配置**时系统校验六维占比**合计须恰好等于 100%**（不可大于或小于 100%），否则拒绝保存并提示当前合计值。

|配置项|说明|
|---|---|
|六维度占比|整数百分比，默认 60 / 15 / 8 / 2 / 5 / 10；可调整为如交易 70%、有效持仓 10% 等|
|占比合计|实时展示当前六维之和；等于 100% 时绿色，否则红色|
|有效持仓最小时长|正整数 + 单位（小时或分钟），默认 `1 小时`；仅统计持仓时长超过该阈值的仓位|
|保存子池配置|校验占比合计 = 100% 后写入配置并记录操作日志|
