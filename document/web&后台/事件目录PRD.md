...
> **指标口径**：下文 **保证金率** = 账户权益 ÷ 维持保证金 × 100%（数值越高越安全；**≤100%** 触发强平/强减，与持仓页「联合保证金率」风控口径一致，具体公式以产研风控规则为准）。

| 序号 | 事件 | 说明 | 触发 | 接收人 | 幂等 |
|---|---|---|---|---|---|
| 1 | `contract.liquidation.executed` | 强平 | 用户合约仓位 **强制平仓** 执行成功（保证金率 **≤100%** 触发） | 被强平 UID | UID + 强平流水 ID |
| 2 | `contract.adl.executed` | 阶梯减仓（ADL） | **ADL** 机制对用户仓位执行 **阶梯减仓** 成交成功 | 被减仓 UID | UID + ADL 流水 ID |
| 3 | `contract.liquidation_risk.warning_150` | 强平风险预警 · 升高 | 保证金率 **≤150%** 且为 **48 小时内该档位首次** 触发 | 用户 UID | UID + 档位 + 48h 窗口 ID |
| 4 | `contract.liquidation_risk.warning_110` | 强平风险预警 · 紧急 | 保证金率 **≤110%** 且为 **48 小时内该档位首次** 触发 | 用户 UID | UID + 档位 + 48h 窗口 ID |

**不发通知**：

- 保证金率在预警线附近 **反复波动**、但 48 小时内 **非首次** 跌破同一档位
...
| 档位独立 | **150%** 与 **110%** 两档 **各自** 维护 48 小时计时 |
| 首次触发 | 某档位计时器为 0 且保证金率 **首次 ≤ 该档阈值** → 发送对应通知，**计时器重置为 0 并重新开始 48 小时倒计时** |
| 窗口内重复 | 48 小时内同一档位再次跌破阈值 → **不发**（避免刷屏） |
| 窗口结束 | 48 小时届满后计时器回到可触发状态；若再次首次跌破该档 → 可再次发送 |
| 跨档 | 同一时刻两档均满足时：**两档各自判断**；若均为首次，**可同时发送**（先 150 后 110，或合并策略以实现为准，默认各发各的） |

---

#### 1 · `contract.liquidation.executed` · 强平

**业务触发：** 《[持仓管理 & 资金流水 模块](持仓管理%20&%20资金流水%20模块.md)》§3.1 当前持仓 · **联合保证金率** 说明（≤100% 触发强减或强平）；强平成交写入持仓/流水后向用户发送通知。底层强平逻辑以产研风控规则为准（见《合约交易页》重要说明）。
...
#### 3 · `contract.liquidation_risk.warning_150` · 强平风险预警 · 升高

**业务触发：** 合约风控引擎 **实时监测** 用户保证金率；当保证金率 **≤150%** 且满足 **48 小时内该档位首次触发** 规则时发送（见本节「强平风险预警 · 48 小时首次触发规则」）。展示口径见《[APP 端合约交易页](../app/APP%20端合约交易页.md)》§4.2.7 联合保证金率说明。
...
#### 4 · `contract.liquidation_risk.warning_110` · 强平风险预警 · 紧急

**业务触发：** 合约风控引擎 **实时监测** 用户保证金率；当保证金率 **≤110%** 且满足 **48 小时内该档位首次触发** 规则时发送（见本节「强平风险预警 · 48 小时首次触发规则」）。
