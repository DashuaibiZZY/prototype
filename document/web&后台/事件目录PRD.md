...
| 3 | `fee_rate.custom_config.applied` | 设置自定义费率 | 运营 **自定义 Maker/Taker** 配置，三级审批 **全部通过** 并生效 | 目标 UID | 审批单 ID + UID |
| 4 | `fee_rate.config.expired` | 设置的费率失效 | 运营配置 **有效期到期**（到期日 **24:00 UTC+8**），自动恢复自然升降 | 目标 UID | UID + 运营配置记录 ID |
...
- 存在 **运营自定义费率** 期间的自然升降评估（用户冻结在自然通道外）

> **自然升降 vs 运营配置**：仅当用户处于 **自然升降通道**（无运营自定义费率）时，才可能触发 `fee_rate.vip.upgraded` / `fee_rate.vip.downgraded`。

---

#### 1 · `fee_rate.vip.upgraded` · VIP 升级

**业务触发：** 《[VIP & 费率](VIP & 费率.md)》§1.1 **VIP 自然升降级** · 每日 **UTC+8 24:00** 结算后，用户处于自然升降通道，**实际生效 VIP 等级** 较上一日升档。
...
#### 3 · `fee_rate.custom_config.applied` · 设置自定义费率

**业务触发：** 《[VIP & 费率](VIP & 费率.md)》§1.2 **运营后台配置费率** · 运营 **自定义 Maker/Taker**，§1.5 三级审批 **全部通过** 并生效（§3.2 审批通过后的系统行为；老板终审须无持仓）。
...
#### 4 · `fee_rate.config.expired` · 设置的费率失效
