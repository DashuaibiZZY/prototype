...
| 触达渠道 | 仅 **站内信** + **App Push** + **Email**（不支持 SMS 等） |
| 文案结构 | **站内信**、**Push**、**Email** 分别配置，各有独立标题与正文；Push 须更短，不得复用站内信全文 |
...
> **变量说明**：USDC 奖励到账资产类型固定为 USDC；站内信 / Email 正文仍使用 `{{ currency }}` 占位，Push 正文硬编码 USDC。
