...
建议您尽快采取以下措施：
1. 登录 App 或 Web，前往「合约交易」查看持仓详情与联合保证金率；
2. 适当 **降低杠杆** 或 **减少仓位**，释放占用保证金；
3. 向合约账户 **追加保证金**，提升账户权益；
4. 设置止盈止损，避免单边行情导致风险快速累积。
...
请立即执行以下操作（任选或组合）：
1. **立即减仓**：优先平掉部分或全部高风险仓位，快速释放保证金；
2. **追加保证金**：向合约账户转入 USDC，提升账户权益；
3. **降低杠杆**：调整新开仓或现有仓位的杠杆倍数（如适用）；
4. 打开 App / Web 合约页，实时关注保证金率变化，直至回落至安全区间。
