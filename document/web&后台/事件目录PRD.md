...
| 触达渠道 | 仅 **站内信** + **App Push** + **Email**（不支持 SMS 等）；§3.6 合约类事件 **默认含 Email**，其余业务线默认不含 |
...
> **单条事件结构**：业务触发 → 变量表 → **站内信**（标题 + 正文）→ **Push**（标题 + 正文）→（§3.6 另含 **Email**）→ 默认配置。
...
| 4 | `partner.application.rejected` | 申请被拒 | 后台 **驳回合伙人计划申请** 并填写驳回原因 | 申请人 UID | UID + 申请单 ID |

**不发通知**：审批流转（运营/风控侧）、日结放款到账（本期不做）。
...
#### 4 · `partner.application.rejected` · 申请合伙人被拒

**业务触发：** 《[邀请合伙人无限层返佣系统 · 后台管理](邀请合伙人无限层返佣系统-后台管理.md)》§2.7.4 **驳回合伙人申请** · 运营填写 **驳回原因（必填）** 并确认后，申请单状态变为「已驳回」。
...
|---|---|
| `{{ uid }}` | 接收通知的用户 UID（申请人） |
| `{{ reject_reason }}` | 驳回原因（与用户端「已驳回」页展示一致） |
| `{{ application_id }}` | 合伙人申请单 ID（系统字段，正文不展示） |
...
```
尊敬的用户，您的合伙人计划申请未通过审核。

驳回原因：{{ reject_reason }}

您可补充材料后重新提交申请。请前往「合伙人计划」页查看详情并重新申请。
```

**Push 标题：** 合伙人申请未通过

**Push 正文：** 申请未通过，请查看驳回原因并重新提交
...
**Lark 文档（运营维护，消息文案可对照）**：https://ksgf73ukjokp.sg.larksuite.com/wiki/BiTown38EipLF3k8TJllGd35gHc  
...
| 5 | `trial_fund.activate_deadline_remind_1d` | 卡券激活提醒 · 1 天 | **卡券有效期** 截止前 **1 天**，卡券仍为「可使用」 | 收券 UID | UID + 卡券 ID + 提醒档位 `1d` |
| 6 | `trial_fund.activate_deadline_remind_1h` | 卡券激活提醒 · 1 小时 | **卡券有效期** 截止前 **1 小时**，卡券仍为「可使用」 | 收券 UID | UID + 卡券 ID + 提醒档位 `1h` |
| 7 | `trial_fund.position_deadline_remind_3d` | 开仓有效期提醒 · 3 天 | **开仓有效期** 截止前 **3 天**，卡券仍为「已使用」 | 用户 UID | UID + 卡券 ID + 提醒档位 `3d` |
| 8 | `trial_fund.position_deadline_remind_1d` | 开仓有效期提醒 · 1 天 | **开仓有效期** 截止前 **1 天**，卡券仍为「已使用」 | 用户 UID | UID + 卡券 ID + 提醒档位 `1d` |
| 9 | `trial_fund.position_deadline_remind_1h` | 开仓有效期提醒 · 1 小时 | **开仓有效期** 截止前 **1 小时**，卡券仍为「已使用」 | 用户 UID | UID + 卡券 ID + 提醒档位 `1h` |
...
| 规则 | 说明 |
|---|---|
| 卡券状态 | 激活截止提醒（序号 5、6）仅对「**可使用**」；开仓截止提醒（序号 7–9）仅对「**已使用**」 |
| 发送时机 | 定时任务在对应截止时间点前 **3 天 / 1 天 / 1 小时** 各发送一次 |
| 幂等 | 同一卡券同一提醒档位 **仅发送一次**；已激活 / 已过期 / 已失效后 **不再发送** |
| 渠道 | 站内信 + App Push |
...
**业务触发：** 《[合约体验金&卡券中心页（后台/Web/APP）](合约体验金&卡券中心页（后台_Web_APP）.md)》§6.2 **审批通过后的系统行为** · 批量发放四级审批全部通过，卡券写入用户卡券中心（状态「可使用」）。
...
**业务触发：** 《[合约体验金&卡券中心页（后台/Web/APP）](合约体验金&卡券中心页（后台_Web_APP）.md)》§二 **卡券状态** · **可使用 → 已过期**：卡券有效期结束，用户未激活。
...
**业务触发：** 《[合约体验金&卡券中心页（后台/Web/APP）](合约体验金&卡券中心页（后台_Web_APP）.md)》§二 **卡券状态** · **可使用 → 已使用**：用户在卡券有效期内点击激活，体验金注入合约账户。
...
**业务触发：** 《[合约体验金&卡券中心页（后台/Web/APP）](合约体验金&卡券中心页（后台_Web_APP）.md)》§三 **回收机制** · **回收标记**（开仓有效期到期）：状态「已使用」→「已失效」，可用计入累计已回收，占用转待回收。
...
#### 5 · `trial_fund.activate_deadline_remind_1d` · 卡券激活提醒 · 1 天

**业务触发：** 《[合约体验金&卡券中心页（后台/Web/APP）](合约体验金&卡券中心页（后台_Web_APP）.md)》§1.2 **到期提醒** · 定时任务扫描状态「可使用」卡券，距 **卡券激活截止时间** 剩余 **1 天** 时发送（见本节「到期前提醒 · 发送规则」）。
...
请在 {{ activate_deadline }}（UTC+8）前前往卡券中心激活，距截止还剩 {{ remind_offset }}。
```

**Push 标题：** 体验金即将过期

**Push 正文：** {{ amount }} USDC 体验金还剩 {{ remind_offset }}，请激活

**默认配置：** 业务通知 · 普通 · 站内信 + App Push

---

#### 6 · `trial_fund.activate_deadline_remind_1h` · 卡券激活提醒 · 1 小时

**业务触发：** 《[合约体验金&卡券中心页（后台/Web/APP）](合约体验金&卡券中心页（后台_Web_APP）.md)》§1.2 **到期提醒** · 定时任务扫描状态「可使用」卡券，距 **卡券激活截止时间** 剩余 **1 小时** 时发送（见本节「到期前提醒 · 发送规则」）。
...
**Push 正文：** {{ amount }} USDC 体验金还剩 {{ remind_offset }}，请尽快激活

**默认配置：** 业务通知 · 较高 · 站内信 + App Push

---

#### 7 · `trial_fund.position_deadline_remind_3d` · 开仓有效期提醒 · 3 天

**业务触发：** 《[合约体验金&卡券中心页（后台/Web/APP）](合约体验金&卡券中心页（后台_Web_APP）.md)》§1.2 **到期提醒** · 定时任务扫描状态「已使用」卡券，距 **开仓有效期截止时间** 剩余 **3 天** 时发送（见本节「到期前提醒 · 发送规则」）。
...
开仓有效期将于 {{ position_valid_until }}（UTC+8）结束，距回收还剩 {{ remind_offset }}，请尽快完成交易使用。
```

**Push 标题：** 体验金即将回收

**Push 正文：** {{ amount }} USDC 体验金还剩 {{ remind_offset }}，请尽快使用

**默认配置：** 业务通知 · 普通 · 站内信 + App Push

---

#### 8 · `trial_fund.position_deadline_remind_1d` · 开仓有效期提醒 · 1 天

**业务触发：** 《[合约体验金&卡券中心页（后台/Web/APP）](合约体验金&卡券中心页（后台_Web_APP）.md)》§1.2 **到期提醒** · 定时任务扫描状态「已使用」卡券，距 **开仓有效期截止时间** 剩余 **1 天** 时发送（见本节「到期前提醒 · 发送规则」）。
...
#### 9 · `trial_fund.position_deadline_remind_1h` · 开仓有效期提醒 · 1 小时

**业务触发：** 《[合约体验金&卡券中心页（后台/Web/APP）](合约体验金&卡券中心页（后台_Web_APP）.md)》§1.2 **到期提醒** · 定时任务扫描状态「已使用」卡券，距 **开仓有效期截止时间** 剩余 **1 小时** 时发送（见本节「到期前提醒 · 发送规则」）。
...
**业务触发：** 《[VIP & 费率](VIP & 费率.md)》§1.1 **VIP 自然升降级** · 每日 **UTC+8 24:00** 结算后，用户处于自然升降通道（或 VIP 1 保底升档），**实际生效 VIP 等级** 较上一日升档。
...
**业务触发：** 《[VIP & 费率](VIP & 费率.md)》§1.1 **VIP 自然升降级** · 每日 **UTC+8 24:00** 结算后，用户处于自然升降通道，近 14 日交易量不达标 **立即滑档**，实际生效等级较上一日降档。
...
**业务触发：** 《[VIP & 费率](VIP & 费率.md)》§1.2 **运营后台配置费率** · 配置方式为 **指定 VIP 等级**（VIP 1–4），§1.5 三级审批 **全部通过** 并生效（§3.2 审批通过后的系统行为）。
...
**业务触发：** 《[VIP & 费率](VIP & 费率.md)》§1.2 **运营后台配置费率** · 配置方式为 **自定义 Maker/Taker**，§1.5 三级审批 **全部通过** 并生效（§3.2 审批通过后的系统行为；老板终审须无持仓）。
...
**业务触发：** 《[VIP & 费率](VIP & 费率.md)》§1.2 **有效期** · 运营配置填写了有效期天数，到期日 **24:00（UTC+8）** 自动失效，用户恢复 VIP 自然升降级（§1.2 生效规则 · 若配置了有效期天数，到期自动失效）。
...
**业务 PRD**：《[合约交易页](合约交易页.md)》·《[持仓管理 & 资金流水 模块](持仓管理%20&%20资金流水%20模块.md)》·《[APP 端合约交易页](../app/APP%20端合约交易页.md)》  
**默认**：业务通知 · 站内信 + App Push + **Email**

> **指标口径**：下文 **保证金率** = 账户权益 ÷ 维持保证金 × 100%（数值 **从低位向 100% 递增**；**越接近 100% 越接近强平**；**达到 / 触及 100%** 触发强平/强减，与持仓页「联合保证金率」风控口径一致，具体公式以产研风控规则为准）。

| 序号 | 事件 | 说明 | 触发 | 接收人 | 幂等 |
|---|---|---|---|---|---|
| 1 | `contract.liquidation.executed` | 强平 | 用户合约仓位 **强制平仓** 执行成功（保证金率 **达到 / 触及 100%** 触发） | 被强平 UID | UID + 强平流水 ID |
| 2 | `contract.adl.executed` | 阶梯减仓（ADL） | **ADL** 机制对用户仓位执行 **阶梯减仓** 成交成功 | 被减仓 UID | UID + ADL 流水 ID |
| 3 | `contract.liquidation_risk.warning_80` | 强平风险预警 · 升高 | 保证金率 **≥80%** 且为 **48 小时内该档位首次** 触发 | 用户 UID | UID + 档位 + 48h 窗口 ID |
| 4 | `contract.liquidation_risk.warning_90` | 强平风险预警 · 紧急 | 保证金率 **≥90%** 且为 **48 小时内该档位首次** 触发 | 用户 UID | UID + 档位 + 48h 窗口 ID |

**不发通知**：

- 保证金率在预警线附近 **反复波动**、但 48 小时内 **非首次** 触及同一档位
- 强平/ADL **执行失败** 或仅内部重试、未产生用户可见成交
- 普通下单、成交、资金费率结算、止盈止损触发等（本期不做）

#### 强平风险预警 · 48 小时首次触发规则

| 项 | 规则 |
|---|---|
| 统计对象 | 按 **UID**（全仓按账户维度；逐仓可按仓位维度，以实现为准） |
| 档位独立 | **80%** 与 **90%** 两档 **各自** 维护 48 小时计时 |
| 首次触发 | 某档位计时器为 0 且保证金率 **首次 ≥ 该档阈值** → 发送对应通知，**计时器重置为 0 并重新开始 48 小时倒计时** |
| 窗口内重复 | 48 小时内同一档位再次触及阈值 → **不发**（避免刷屏） |
| 窗口结束 | 48 小时届满后计时器回到可触发状态；若再次首次触及该档 → 可再次发送 |
| 跨档 | 同一时刻两档均满足时：**两档各自判断**；若均为首次，**可同时发送**（先 80 后 90，或合并策略以实现为准，默认各发各的） |

---

#### 1 · `contract.liquidation.executed` · 强平

**业务触发：** 《[持仓管理 & 资金流水 模块](持仓管理%20&%20资金流水%20模块.md)》§3.1 当前持仓 · **联合保证金率** 说明（达到 / 触及 100% 触发强减或强平）；强平成交写入持仓/流水后向用户发送通知。底层强平逻辑以产研风控规则为准（见《合约交易页》重要说明）。
...
因保证金率达到 100%，系统已对该仓位执行强平。触发时标记价格：{{ mark_price }}
...
强平说明：
当保证金率达到 100% 时，系统将按风控规则对相应仓位执行强制平仓，以控制账户风险。本次强平即因您的保证金率触及强平线而触发。

建议您：
1. 登录 App 或 Web，前往「合约交易」查看当前持仓与账户权益；
2. 复盘本次持仓的杠杆倍数与仓位规模，评估是否超出自身风险承受能力；
3. 后续交易请预留充足保证金，并关注保证金率变化，在风险升高时及时减仓或追加保证金。

如有疑问，可通过帮助中心或在线客服联系我们。
```

**默认配置：** 业务通知 · 较高 · 站内信 + App Push + Email

---

#### 2 · `contract.adl.executed` · 阶梯减仓（ADL）

**业务触发：** 《[持仓管理 & 资金流水 模块](持仓管理%20&%20资金流水%20模块.md)》§3.4 资金账单 · **阶梯减仓** 子类型（ADL 机制触发的平多/平空）；减仓成交成功并写入流水后发送通知。
...
ADL 说明：
当市场出现极端波动、对手方无法及时平仓时，平台会通过 ADL（Auto-Deleveraging，自动减仓）机制，按规则对部分盈利仓位进行阶梯减仓，以维持市场稳定。本次通知即因 ADL 机制对您的仓位执行了部分减仓。

建议您：
1. 登录 App 或 Web，查看剩余持仓数量与账户权益变化；
2. 关注该交易对后续行情与保证金率，评估是否需要进一步调整仓位；
3. 若仍持有同方向仓位，请确保账户留有足够保证金，避免再次触发风控处置。
...
#### 3 · `contract.liquidation_risk.warning_80` · 强平风险预警 · 升高

**业务触发：** 合约风控引擎 **实时监测** 用户保证金率；当保证金率 **≥80%** 且满足 **48 小时内该档位首次触发** 规则时发送（见本节「强平风险预警 · 48 小时首次触发规则」）。展示口径见《[APP 端合约交易页](../app/APP%20端合约交易页.md)》§4.2.7 联合保证金率说明。
...
|---|---|
| `{{ uid }}` | 接收通知的用户 UID |
| `{{ warning_threshold }}` | 本档预警阈值（固定 `80`） |
...
您的保证金率已升至 80%（强平线为 100%）。建议及时减仓或追加保证金，避免继续上升触发强平。
...
我们检测到您的合约账户保证金率已升至 80%，账户风险正在升高。

保证金率说明：
保证金率从低位向 100% 递增，数值越接近 100%，越接近强制平仓（强平线）。当前您已触及 80% 预警线，若行情继续不利或仓位规模过大，保证金率可能进一步上升。

请您及时补充保证金，或降低您的持仓。

请在保证金率进一步上升前完成处理。强平线为 100%，触及后将按规则强制平仓。
...
#### 4 · `contract.liquidation_risk.warning_90` · 强平风险预警 · 紧急

**业务触发：** 合约风控引擎 **实时监测** 用户保证金率；当保证金率 **≥90%** 且满足 **48 小时内该档位首次触发** 规则时发送（见本节「强平风险预警 · 48 小时首次触发规则」）。
...
|---|---|
| `{{ uid }}` | 接收通知的用户 UID |
| `{{ warning_threshold }}` | 本档预警阈值（固定 `90`） |
| `{{ liquidation_threshold }}` | 强平阈值（固定 `100`） |
| `{{ margin_mode }}` | 保证金模式：`cross` / `isolated` |
| `{{ symbol }}` | 触发关联交易对；全仓账户级预警时可为空 |
| `{{ occurred_at }}` | 事件发生时间（UTC+8，系统字段，正文不展示） |
...
您的保证金率已升至 90%（强平线为 100%）。请立即减仓或追加保证金，否则将很快触发强制平仓。
...
紧急提醒：您的合约账户保证金率已升至 90%，已非常接近 100% 强平线，请立即处理。

当前状态：
保证金率越接近 100%，被强制平仓的风险越高。您已触及 90% 紧急预警线，若未及时处理，保证金率继续上升将触发系统强平。

请您及时补充保证金，或降低您的持仓。

强平线为 100%。一旦触及，系统将按风控规则对相应仓位执行强制平仓，可能造成较大损失。

如有疑问，请尽快通过在线客服获取协助。
```

**默认配置：** 业务通知 · 紧急 · 站内信 + App Push + Email
