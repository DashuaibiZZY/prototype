...
|涨幅榜| `行情.html?view=gainers` |
|跌幅榜| `行情.html?view=losers` |

**涨跌榜跳转处理逻辑**

1. 页面加载时解析 `view=gainers` 或 `view=losers`。
2. 列表展示**全部已上线交易对**，并按「今日涨跌」预排序：
    - `gainers`：涨跌幅从高到低（涨幅最大在前）
    - `losers`：涨跌幅从低到高（跌幅最大在前）
3. 表头「今日涨跌」列排序箭头呈黑色高亮，方向与预排序一致。
4. 该跳转指令**仅当次生效**；用户手动切换标签 Tab 或更改排序后，以最新操作为准。

**普通 Tab 跳转**

- 携带 `tab` 时，打开行情页并选中对应单层标签 Tab（全部 / 新币上线 / 热门 / 自定义标签）。
