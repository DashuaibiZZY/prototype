...
| Tab | 说明 |
|---|---|
| **自选** | 固定排在 **「全部」左侧**；仅展示用户已收藏交易对；无自选时见 §2.5 空状态 |
| **全部** | 展示所有已上线交易对；`priority` 在可配置标签中固定最高（默认 `0`） |
...
- 默认选中「全部」；从首页「收藏 · 查看更多」进入时选中「自选」（`tab=fav`）。
...
- 位置：分类标签栏 **最左侧**（「全部」左边），为系统固定 Tab，见《[交易对标签配置及填充逻辑](../交易对标签配置及填充逻辑.md)》§2。
- 列表：仅展示用户已收藏交易对；列表项左侧可展示星标；字段与 §2.3 一致。
- 空状态：无自选时展示插图与引导文案（如「暂无自选」），引导用户至详情页星标添加。

**添加 / 移除自选**

- **添加**：在行情详情页点击星标，或（首页）推荐一键收藏；收藏后出现在「自选」Tab。
- **移除**：在详情页取消星标；或在 **编辑自选** 子页批量勾选删除（见下）。

**编辑自选（批量管理，仅行情页入口）**

- **入口**：在 **自选 Tab** 且列表非空时，表头「今日涨跌」列右侧展示 **铅笔编辑图标**（**不展示「编辑自选」四字文案**）。
- **跳转**：点击图标进入 **编辑自选** 子页（原型见文档顶部链接）。
- **子页能力**：勾选多条、全选、批量删除；单行置顶、拖动排序（排序结果持久化，与自选 Tab 列表顺序一致）。
- **说明**：合约交易页「选择合约」弹层内的「自选」Tab **仅展示**已收藏列表，**无**编辑图标；批量编辑统一在行情页自选 Tab 完成。

**排序**

- 自选 Tab 默认按用户保存的排序展示（如编辑页拖动结果）；新收藏可追加或按产品规则插入（与编辑页操作一致）。
...
|参数|说明|示例|
|---|---|---|
|`tab`|标签 Tab：`fav` / `all` / `hot` / `new` / 自定义标签 key| `tab=fav` · `tab=hot` |
...
- 携带 `tab`（不含 `view`）时，打开行情页并选中对应单层标签 Tab（**自选** / 全部 / 新币上线 / 热门 / 自定义标签）。
