...
|Tab|说明|
|---|---|
|收藏|展示用户已收藏的交易对；下方展示行业筛选条；显示「编辑自选」按钮|
|热门|展示平台综合热度 Top 交易对；下方展示行业筛选条（全部 / 数字货币 / 股票 / 大宗商品）|
...
- 一级行业 Tab、二级分类 Tab 均支持配置 `priority` 整数字段，**0 为最高优先级**，数值越小越靠前展示。
- 前端按 `priority` 升序渲染 Tab 顺序；同优先级保持稳定排序。
- 详细字段见《APP 端首页》1.3 节「交易对配置」。

**二级分类标签（仅「数字货币」「大宗商品」下展示，或收藏/热门下的行业筛选）**

- 由后台管理系统配置，前端根据返回的标签列表动态渲染。示例：热门、新币上线、主流币、AI、TradeFi 等。

- 每个二级标签可配置 `priority`（0 为最高优先级），前端按升序排列展示。

- 后台下架某标签则隐藏；新增标签需配置优先级后参与排序。

- 点击二级标签，仅展示该行业下带有对应标签的交易对。

**交易对配置关联**

- 每个交易对需配置「行业分类标签」，决定其归属的一级 Tab。

- 原有的动态分类标签（热门、新币等）作为二级标签，挂载在「数字货币」或「大宗商品」一级 Tab 下。
