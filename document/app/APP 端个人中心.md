...
- `forx://profile` → 个人中心
- `forx://profile/detail` → 个人资料
- `forx://settings` → 设置
- `forx://settings/authenticator` → 身份验证器（邮箱校验）

---

## 7\. 数据接口（产品字段说明，供技术对齐）

以下为用户中心聚合接口建议字段，具体 REST 路径由技术定义：

| 字段 | 说明 |
|---|---|
| `uid` | 用户 UID |
| `loginType` | `email` / `wallet` |
| `email` | 邮箱登录时完整邮箱；钱包登录为空 |
| `emailMasked` | 展示用脱敏邮箱 |
| `walletAddress` | 钱包登录：连接地址；邮箱登录：Privy 地址 |
| `walletAddressShort` | 简化地址 |
| `vipLevel` | 当前 VIP 等级数字或名称 |
| `makerFeeRate` | 当前 Maker 费率 |
| `takerFeeRate` | 当前 Taker 费率 |
| `totpBound` | 是否已绑定 TOTP |
| `currencyPreference` | 用户币种偏好 |
...
- **性能**：个人中心首屏建议 500ms 内展示骨架，1s 内完成主信息渲染（弱网除外）。
- **无障碍**：复制按钮、返回、设置需有足够点击热区（≥ 44pt）；验证码输入框需支持系统密码自动填充禁用，避免与短信验证码混淆。
- **安全**：复制完整地址/邮箱仅在用户主动点击复制后写入剪贴板；敏感页禁止截屏策略由客户端统一安全策略决定。
- **埋点**（建议）：`profile_hub_view`、`profile_detail_view`、`settings_view`、`totp_bind_start`、`totp_bind_success`、`totp_remove_success` 及各入口点击 `source=profile_hub`。

---

## 9\. 与旧版「我的」原型关系

仓库内 `我的.html` 为早期深色全功能交互 spec（含资产卡、多账户、消息铃铛等），**非**当前线上结构。当前产品拆分：

- 资产相关 → 底部 Tab「资产」
- 账户身份与权益导航 → **个人中心**（本文档）
- 旧页保留仅供历史对照，PRD 与开发以本文档及对应浅色原型为准。
...
| 版本 | 日期 | 说明 |
|---|---|---|
| v1.0 | 2026-08-03 | 首版：个人中心、个人资料、设置、身份验证器全流程；邮箱/钱包互斥；无截图 |
