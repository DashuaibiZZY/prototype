...
| **邮箱登录** | 注册/登录邮箱 | Privy 为账号**自动生成**的链上地址（非用户自行关联外部钱包） | **展示**绑定/管理入口 |

邮箱登录用户同时拥有「邮箱」与「Privy 钱包地址」两条信息，均在个人资料页展示并可复制；该钱包地址用于平台内链上相关能力，**不等于**钱包登录态下的「已连接钱包」。
...
#### 3\.3\.3 邮箱登录 · 钱包地址卡片（Privy）

| 字段 | 说明 |
|---|---|
| 标签 | `钱包地址` |
| 内容 | 简化 Privy 地址，如 `0x8b2e...4f1a` |
| 辅助说明 | 小字：`由 Privy 自动生成，无需关联外部钱包` |
| 操作 | 「复制」→ 完整地址 |

#### 3\.3\.4 UID 卡片（两种登录均展示）
...
**业务说明**：身份验证器用于账户安全增强（动态验证码/TOTP），**不强制绑定于提币流程**；具体在哪些操作触发验证由安全策略配置，本页仅展示绑定状态与入口。
