...
|标签|说明|
|---|---|
|當前委託\(n\)|当前活跃委托，含基础单与止盈止损单子 Tab；计数含两类子 Tab 合计|
|歷史委託|已结束委托记录|
|持倉歷史|已关闭仓位记录|
|歷史成交|逐笔成交明细|
|資金流水|账户资金变动汇总|
...
与 App 合约交易页「当前委托 · 基础单」卡片一致（含修改参数、撤单、止盈止损关联设置等交互）。
...
|头部|撤單按钮|撤销该笔委托|
|标签行|方向标签|统一为 `买入开多` / `卖出开空` / `卖出平多` / `买入平空`；开多/平空绿色，开空/平多红色|
|标签行|仓位模式、杠杆|如 `全仓` `3X`|
...
原型 Mock：2 条基础单（BNBUSDC 卖出平多 + TP/SL；ETHUSDT 买入平空）。
...
|标签行|方向、仓位模式、杠杆、时间|同基础单；委托时间靠右，格式 `06/11 18:59:34`|
|明细|委托数量(USDT)|委托名义价值|
