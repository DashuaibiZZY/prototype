...
const shots = [
  ...
  {
    file: 'admin-bonus-config.png',
    url: 'file://' + path.join(ROOT, 'perp_dex/admin/积分后台.html'),
    hash: 'bonus',
    before: "(() => { const p = document.getElementById('bonus-activity-picker'); if (p) p.scrollIntoView({ block: 'center' }); })()",
    selector: '#page-bonus .card',
  ...
  {
    file: 'admin-manual-grant.png',
    url: 'file://' + path.join(ROOT, 'perp_dex/admin/积分后台.html'),
    hash: 'manual',
    before: "(() => { const p = document.getElementById('points-activity-picker'); if (p) p.scrollIntoView({ block: 'center' }); })()",
    selector: '#page-manual .card',
    viewport: { w: 1440, h: 900 },
    clipHeight: 720,
  },
